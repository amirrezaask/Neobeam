...
pub fn encode(key: TermKey, shift: bool, ctrl: bool, alt: bool, app_cursor: bool) -> Option<Vec<u8>> {
    ...
    let mut out = if modified && matches!(key, TermKey::Up | TermKey::Down | TermKey::Left | TermKey::Right | TermKey::Home | TermKey::End | TermKey::Insert | TermKey::Delete | TermKey::PageUp | TermKey::PageDown | TermKey::F(_)) {
        Vec::new()
    } else {
        alt_prefix.to_vec()
    };
...
mod tests {
    ...
    fn encodes_ctrl_and_alt_characters() {
        assert_eq!(encode(TermKey::Char('c'), false, true, false, false), Some(vec![3]));
        assert_eq!(
            encode(TermKey::Char('x'), false, false, true, false),
            Some(b"\x1bx".to_vec())
        );
        assert_eq!(encode(TermKey::Char(' '), false, true, false, false), Some(vec![0]));
        assert_eq!(encode(TermKey::Char('['), false, true, false, false), Some(vec![0x1b]));
    }

    #[test]
    fn encodes_navigation_and_shift_tab() {
        assert_eq!(encode(TermKey::Up, false, false, false, false), Some(b"\x1b[A".to_vec()));
        assert_eq!(encode(TermKey::Up, false, false, false, true), Some(b"\x1bOA".to_vec()));
        assert_eq!(encode(TermKey::Left, false, true, false, false), Some(b"\x1b[1;5D".to_vec()));
        assert_eq!(
